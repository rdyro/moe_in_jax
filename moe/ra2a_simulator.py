...
def ragged_all_to_all(x: jax.Array, out: jax.Array, input_offsets: jax.Array, send_sizes: jax.Array,
                      output_offsets: jax.Array, recv_sizes: jax.Array, *, axis_name: str = "x") -> jax.Array:
  out_spec = P(axis_name, *[None for _ in range(x.ndim - 1)])
  meta = RA2AMeta(input_offsets=input_offsets, send_sizes=send_sizes,
                  output_offsets=output_offsets, recv_sizes=recv_sizes)

  def check_vma(x):
    if len(x.sharding.spec) == 0:
      return False
    vma = x.sharding.spec[0]
    if vma is None:
      return False
    if isinstance(vma, str):
        return vma == axis_name
    else:
        return axis_name in vma

  passes_vma = jax.tree.map(check_vma, (x, out, meta))
  if not all(jax.tree.leaves(passes_vma)):
    raise ValueError(f"ragged_all_to_all requires that the all inputs specs have axis_name sharding the first axis,"
                     f" but\n  ---> {passes_vma = }")

  @partial(jax.shard_map, out_specs=out_spec)
  def fn(x, out, meta):
    axis_index = jax.lax.axis_index(axis_name)
    x_all = jax.lax.all_gather(x, axis_name=axis_name, tiled=False)
    meta_all: RA2AMeta = jax.lax.all_gather(meta, axis_name=axis_name, tiled=False)
    ...
    def insert(i, buf):
      existing_slice = jax.lax.dynamic_slice_in_dim(buf, meta_all.output_offsets[i, axis_index], x.shape[0])
      x_ = jnp.roll(x_all[i, ...], -meta_all.input_offsets[i, axis_index])
