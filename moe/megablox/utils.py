...
def _get_scale_tile_info(
  ...
  return ScalesTilingInfo(scales_per_tile, scales_tile_size, tile_idx_to_scales_tile_idx)


def quant_block_spec(x: QArray, x_spec: pl.BlockSpec, reduction_axis: int) -> tuple[QArray, QArray]:
  ...
  scale_info_per_axis = [
    ...
    for eps, tile_size, s, mas in zip(eps_list, tile_sizes, x_values.shape, min_addressable_sizes)
  ]
  ...
  scales_blockspec = dataclasses.replace(x_spec, block_shape=scales_block_shape, index_map=index_map)
  ...
  **kw,
  ...
  def pallas_call(*args):
    ...
    def pipeline(*args_refs):
      ...
      grid = tuple(d[0] if d is not None else s for d, s in zip(args_refs[0], grid_static))
